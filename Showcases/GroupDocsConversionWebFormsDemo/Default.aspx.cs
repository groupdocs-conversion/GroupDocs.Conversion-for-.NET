...
namespace GroupDocsConversionWebFormsDemo
{
    public partial class _Default : Page
    {
        ...
        protected void ConvertCommand(object sender, CommandEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    convertResult = conversionHandler.Convert<string>(inputFileName, new PdfSaveOptions { OutputType = OutputType.String });
                }
                if (e.CommandArgument.ToString().ToLower().Equals("xlsx"))
                {
                    convertResult = conversionHandler.Convert<string>(inputFileName, new CellsSaveOptions { OutputType = OutputType.String });
                }
                else if (e.CommandArgument.ToString().ToLower().Equals("jpg"))
                {
                    IList<string> convertResults = conversionHandler.Convert<IList<string>>(inputFileName, new ImageSaveOptions { ConvertFileType = ImageSaveOptions.ImageFileType.Jpg, OutputType = OutputType.String });
                ...
                {
                    // Conver to Image format returns multiple images path as string array, one image for each input document page.
                    IList<string> convertResults = conversionHandler.Convert<IList<string>>(inputFileName, new ImageSaveOptions { ConvertFileType = ImageSaveOptions.ImageFileType.Png, OutputType = OutputType.String });
                    foreach (string s in convertResults)
                    {
                        if (!s.Equals(""))
                        {
                ...
                {
                    convertResult = conversionHandler.Convert<string>(inputFileName, new SlidesSaveOptions { OutputType = OutputType.String });
                }
                else if (e.CommandArgument.ToString().ToLower().Equals("html"))
                {
                    convertResult = conversionHandler.Convert<string>(inputFileName, new HtmlSaveOptions { OutputType = OutputType.String });
                }
