...
using GroupDocsConversionVisualStudioPlugin.GUI;
using GroupDocsConversionVisualStudioPlugin.Properties;

namespace GroupDocsConversionVisualStudioPlugin
{
    public class Connect : Extensibility.IDTExtensibility2, IDTCommandTarget
    {
        ...
        private const string GROUPDOCS_MENU_TOOLTIP = "Download and Run GroupDocs.Conversion for .NET API Examples";

        private const string Conversion_COMMAND_NAME = "GroupDocsConversionVSPlugin";
        private const string Conversion_COMMAND_CAPTION = "New GroupDocs.Conversion Example Project";
        private const string Conversion_COMMAND_TOOLTIP = "Download and Run GroupDocs.Conversion for .NET API Examples";
            ...
            try
            {
                ...
                for (int iloop = 1; iloop <= toolsPopup.CommandBar.Controls.Count; iloop++)
                {
                    if (toolsPopup.CommandBar.Controls[iloop].Caption == GROUPDOCS_MENU_CAPTION)
                    {
                        ...
                        {
                            if (cmdbtn.Caption == Conversion_COMMAND_CAPTION)
                            {
                                oBtn = cmdbtn;
                            }
                        }
                ...
                if (oBtn == null)
                {
                    ...
                    Command myCommand = commands.AddNamedCommand2(addInInstance, Conversion_COMMAND_NAME,
                        Conversion_COMMAND_CAPTION, Conversion_COMMAND_TOOLTIP, false, Resources.pnglogosmall, ref contextGUIDS,
           ...
           ref object varOut, ref bool handled)
        {
            try
            {
                ...
                {
                    if (cmdName == addInInstance.ProgID + "." + Conversion_COMMAND_NAME)
                    {
